...
import 'package:validity_phone_number/src/models/country_model.dart';

Future<List<Country>> loadCountries(BuildContext context, {String? locale}) async {
  final supportedLocales = await _loadSupportedLocales(context);

  final actualLocale = locale != null && supportedLocales.contains(locale.toLowerCase()) ? locale : 'en';
...
Future<List<Map<String, dynamic>>> _getCountries(String locale) async {
  try {
    final content = await rootBundle.loadString('packages/validity_phone_number/assets/countries/countries.json');
...
Future<Map<String, dynamic>> _getCountriesNames(String locale) async {
  final content = await rootBundle.loadString('packages/validity_phone_number/assets/names/$locale.json');
